...
class BandwidthMonitor(app_manager.RyuApp):
	...
	for stat in body:
		if stat.port_no in self.topo.ports[name].keys():
			d = self.topo.ports[name][stat.port_no]
			if d in self.topo.hosts:
				h = self.topo.ports[name][stat.port_no]
				self.bwstats.addHostBwStat(h,stat.tx_packets,stat.rx_packets)
